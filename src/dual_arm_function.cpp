                                      ...
                                      bool freeze_waist)   // ★추가 인자
{
    ...
    for (int iter = 0; iter < maxIter; ++iter)
    {
        ...
        VectorXd dq;
        if (k_null > 1e-9) {
            MatrixXd Jpinv = J.transpose() * JJt.inverse();
            MatrixXd N = MatrixXd::Identity(model.nv, model.nv) - Jpinv * J;
            VectorXd phi = k_null * (q_bias - q);
            dq = dq_primary + N * phi;
        } else {
            dq = dq_primary;
        }

        dq(l_shoulder_yaw_idx) = 0.0;
        dq(r_shoulder_yaw_idx) = 0.0;
        if (freeze_waist) dq(waist_idx) = 0.0;
...
VectorXd convertToPinocchioOrder(const double* our_array)
{
    VectorXd q_pin(DoF);
    for (int i = 0; i < DoF; i++) {
        q_pin(ourIdx_to_pinocchioIdx[i]) = our_array[i];
    }
    return q_pin;
}

VectorXd convertToPinocchioOrder(const VectorXd& our_vec)
{
    VectorXd q_pin(DoF);
    for (int i = 0; i < DoF; i++) {
        q_pin(ourIdx_to_pinocchioIdx[i]) = our_vec(i);
    }
    return q_pin;
}

void convertFromPinocchioOrder(const VectorXd& q_pin, double* our_array)
{
    for (int i = 0; i < DoF; i++) {
        our_array[i] = q_pin(ourIdx_to_pinocchioIdx[i]);
    }
}
VectorXd convertFromPinocchioOrder(const VectorXd& q_pin)
{
    VectorXd our_vec(DoF);
    for (int i = 0; i < DoF; i++) {
        our_vec(i) = q_pin(ourIdx_to_pinocchioIdx[i]);
    }
    return our_vec;
}

inline double computeMedianFilter(std::deque<double>& history, double new_value, int window)
{
    history.push_back(new_value);
    if ((int)history.size() > window) history.pop_front();
