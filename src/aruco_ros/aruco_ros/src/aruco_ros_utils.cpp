...
tf2::Transform aruco_ros::arucoMarker2Tf2(const aruco::Marker &marker)
{
  ...
  // Aruco marker의 회전 행렬을 가져옴
  tf2::Matrix3x3 tf_rot(
                        rot.at<double>(0, 0), rot.at<double>(0, 1), rot.at<double>(0, 2), 
                        rot.at<double>(1, 0), rot.at<double>(1, 1), rot.at<double>(1, 2), 
                        rot.at<double>(2, 0), rot.at<double>(2, 1), rot.at<double>(2, 2));

  // Gazebo의 world 좌표계와 동일하게 설정
  tf2::Matrix3x3 combined_rot(
                              1, 0, 0,
                              0, 0, 1,
                              0, -1, 0);

  // 기존 회전 행렬에 combined_rot 적용
  tf_rot = combined_rot * tf_rot;

  // // Roll 축으로 12도 보정하는 행렬
  // double roll_correction_angle = 12.0 * M_PI / 180.0; // 12도를 라디안으로 변환
  // tf2::Matrix3x3 roll_correction(
  //                                cos(roll_correction_angle), -sin(roll_correction_angle), 0,
  //                                sin(roll_correction_angle),  cos(roll_correction_angle), 0,
  //                                0, 0, 1);

  // // 기존 회전 행렬에 roll 보정 적용
  // tf_rot = roll_correction * tf_rot;

  // Translation vector도 동일하게 변환
  tf2::Vector3 tf_orig(
