...
- ✅ 접촉 구간 접근 속도 저감으로 파지 순간 충격/회전 이탈 방지
...
관절 공간 보간 대신 손끝이 3D 직선으로 이동 (5차 시간 스케일링, `v_des` 기준 소요시간 산정). `CartesianLineTrajectory`에 `v_des` 파라미터가 있어 세그먼트별로 다른 속도를 줄 수 있다.
...
- 기본 속도는 `v_des=0.1m/s`지만, **실제 접촉이 일어나는 `standoff → 파지위치` 구간만 `v_des=0.03m/s`로 낮춰서 접근한다.** 물체 표면 안쪽(침투 목표)까지 기본 속도 그대로 들어가면 접촉 순간 물리적 충격이 커서, 좌우 접촉 타이밍/깊이가 조금만 어긋나도 회전 모멘트가 생겨 물체가 굴러 떨어지는 문제가 있었다 (다른 구간은 그대로라 전체 소요시간은 거의 안 늘어남).
